...
function montarTabela(bloco, prefixo){let html=`<table class="tabela-avaliacao"><thead><tr><th>${bloco.titulo}</th><th>SIM</th><th>NÃO</th></tr></thead><tbody>`; bloco.perguntas.forEach((p,i)=>{const n=`${prefixo}_${bloco.titulo.replace(/\W/g,'')}_${i}`; html+=`<tr><td>${p}</td><td><input type="radio" name="${n}" value="Sim"></td><td><input type="radio" name="${n}" value="Não"></td></tr>`;}); return html+'</tbody></table>';}
function calcularIdade(){const v=document.getElementById('dataNascimento').value;if(!v){document.getElementById('idade').value='';return}const nasc=new Date(v+'T00:00:00');const hoje=new Date();let anos=hoje.getFullYear()-nasc.getFullYear();const m=hoje.getMonth()-nasc.getMonth();if(m<0||(m===0&&hoje.getDate()<nasc.getDate())) anos--;document.getElementById('idade').value = anos>=0 ? `${anos} ano(s)` : '';}
function dataBR(valor){if(!valor) return ''; const d=new Date(valor+'T00:00:00'); return `${String(d.getDate()).padStart(2,'0')}/${String(d.getMonth()+1).padStart(2,'0')}/${d.getFullYear()}`;}
function dataParaNomeArquivo(){const valor=document.getElementById('dataAutorizacao').value; const data=valor?new Date(valor+'T00:00:00'):new Date(); return `${String(data.getDate()).padStart(2,'0')}-${String(data.getMonth()+1).padStart(2,'0')}`;}
...
function montarPDF(doc){
  ...
  labelValue('Grau de parentesco:',valor('parentesco'),mx,y,108); labelValue('Data de preenchimento:',dataBR(valor('dataAutorizacao')),114,y,pageW-mx); y+=7;
  linhaManual('Assinatura:',mx,y,pageW-mx); y+=6;
  y=textWrapped('Observação: Na hipótese de guarda compartilhada, recomenda-se a assinatura de ambos os responsáveis legais.',mx,y,pageW-2*mx,7.6,'normal',3.4)+1;
  ...
  y = section('4.DOCUMENTAÇÃO A SER ENTREGUE NO CEREM', y);

doc.setFont('helvetica', 'normal');
doc.setFontSize(8.0);

// O símbolo de conferido é desenhado com linhas, não como caractere Unicode.
...
// Linha 1
const docsLinha1 = [
  { x: mx + 1, texto: 'Cópia da certidão de nascimento da criança.' },
  { x: mx + 69, texto: 'Cópia do documento dos pais/responsáveis' },
  { x: mx + 140, texto: 'Comprovante de residência' }
];

docsLinha1.forEach(item => {
  checkVisual(item.x, y);
  doc.text(item.texto, item.x + 4, y);
});
...
// Linha 2
const docsLinha2 = [
  { x: mx + 1, texto: 'Cartão SUS' },
  { x: mx + 45, texto: 'Caderneta de vacinação' },
  { x: mx + 100, texto: 'Relatórios e exames prévios' }
];

docsLinha2.forEach(item => {
  checkVisual(item.x, y);
  doc.text(item.texto, item.x + 4, y);
});
...
y += 7;
  ...
  y=section('5. AVALIAÇÃO DO NEURODESENVOLVIMENTO PELA ESCOLA',y);
  ...
  doc.setDrawColor(80); doc.rect(mx,y,pageW-2*mx,32);
  ...
  y+=38;
  y=section('6. PROTOCOLO DE RECEBIMENTO NO CEREM',y);
  y=textWrapped('Observação: A escola deve encaminhar essa documentação ao CEREM.',mx,y,pageW-2*mx,7.6,'normal',3.4)+2;
  linhaManual('Responsável pela entrega:',mx,y,138); doc.setFont('helvetica','bold');doc.setFontSize(9.2);doc.text('Data:',143,y); doc.setFont('helvetica','normal');doc.text('______/______/______',154,y); y+=8;
  linhaManual('Responsável pelo recebimento:',mx,y,pageW-mx);
  footer();
}

document.addEventListener('DOMContentLoaded',()=>{popularSelect('nomeEscola',escolas);popularSelect('anoTurma',anos);document.getElementById('dataNascimento').addEventListener('change',calcularIdade);document.getElementById('tabelasPagina1').innerHTML=blocos1.map((b,i)=>montarTabela(b,'p1'+i)).join('');document.getElementById('tabelasPagina2').innerHTML=blocos2.map((b,i)=>montarTabela(b,'p2'+i)).join('');});
