...
let assinaturaDigitalPreenchida = false;
let contextoAssinatura = null;

function configurarAssinaturaDigital(){
  const canvas = document.getElementById('assinaturaDigital');
  if(!canvas) return;
  ...
  function iniciar(evento){
    ...
    desenhando = true;
    assinaturaDigitalPreenchida = true;
    const p = posicao(evento);
    contextoAssinatura.beginPath();
    contextoAssinatura.moveTo(p.x, p.y);
  ...
  function desenhar(evento){
    ...
    evento.preventDefault();
    const p = posicao(evento);
    contextoAssinatura.lineTo(p.x, p.y);
...
function limparAssinatura(){
  ...
  assinaturaDigitalPreenchida = false;
}

function obterAssinaturaDigital(){
  const canvas = document.getElementById('assinaturaDigital');
  if(!canvas || !assinaturaDigitalPreenchida) return null;
...
function montarPDF(doc){
  ...
  labelValue('Grau de parentesco:',valor('parentesco'),mx,y,108); labelValue('Data:',dataBR(valor('dataAutorizacao')),114,y,pageW-mx); y+=7;
  ...
  const assinatura = obterAssinaturaDigital();
  if(assinatura){
    try{doc.addImage(assinatura, 'PNG', xAssinatura+3, y-16, 78, 15);}catch(e){}
  }
  footer();
}

document.addEventListener('DOMContentLoaded',()=>{popularSelect('nomeEscola',escolas);popularSelect('anoTurma',anos);document.getElementById('dataNascimento').addEventListener('change',calcularIdade);document.getElementById('tabelasPagina1').innerHTML=blocos1.map((b,i)=>montarTabela(b,'p1'+i)).join('');document.getElementById('tabelasPagina2').innerHTML=blocos2.map((b,i)=>montarTabela(b,'p2'+i)).join('');configurarAssinaturaDigital();});
