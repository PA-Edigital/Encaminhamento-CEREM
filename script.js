...
function aplicarMascaras(){
  ...
  if(telefone){
    telefone.addEventListener('beforeinput',(e)=>{if(e.data && /\D/.test(e.data)) e.preventDefault();});
    telefone.addEventListener('input',()=>{telefone.value=mascararTelefone(telefone.value);});
    telefone.addEventListener('blur',()=>{telefone.value=mascararTelefone(telefone.value);});
  }
...
function validarFormularioCompleto(){
  ...
  if(somenteDigitos(valor('telefoneResponsavel')).length!==11) erro('telefoneResponsavel','Informe o telefone no formato (35) 988776655.');
...
function montarPDF(doc){
  ...
  labelValue('Grau de parentesco:',valor('parentesco'),mx,y,92); labelValue('Data:',dataBR(valor('dataAutorizacao')),97,y,130); labelValue('Telefone:',valor('telefoneResponsavel'),135,y,pageW-mx); y+=8;
